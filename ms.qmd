...
pacman::p_load(tidyverse, flextable, emmeans, DHARMa, brms, here)
...
# Load packages
pacman::p_load(tidyverse, flextable, brms, ggplot2,lme4, zoo, lmerTest)
...
ungroup()  %>% mutate(temp = gsub("[AB]_", "", trt),
                      cort = gsub("_[2][38]", "", trt))  %>% 
                mutate(group = factor(group, levels = c("R_B", "B_R"))) %>% data.frame()
...
#| warning: false
    ...
    null <- glmer(FC_associative ~ 1 + (1 + Associative_Trial|lizard_id), data = deli_associative, family = binomial(link = "logit"))
GMM_deli <- glmer(FC_associative ~ Associative_Trial*trt + group*Associative_Trial + (1 + Associative_Trial|lizard_id), data = deli_associative, family = binomial(link = "logit"))

GMM_deli2 <- glmer(FC_associative ~temp*cort*Associative_Trial + group*Associative_Trial + (1 + Associative_Trial|lizard_id), data = deli_associative, family = binomial(link = "logit"))
summary(GMM_deli2)

emmeans(GMM_deli2, pairwise ~ temp*Associative_Trial)
emmeans(GMM_deli2, pairwise ~ cort*Associative_Trial)

## Bayesian 
  mod <- brm(FC_associative ~ trt*Associative_Trial + group*Associative_Trial + (1 + Associative_Trial|lizard_id), data = deli_associative, family = bernoulli(link = "logit"), chains = 4, cores = 4, iter = 2000, warmup = 1000, control = list(adapt_delta = 0.99))
  summary(mod)
...
anova(GMM_deli)
plot_deli <- ggplot(deli_associative, aes(x = Associative_Trial, y = FC_associative, color=trt)) +  
geom_smooth(method = "glm", formula = y ~ x, method.args = list(family = "binomial")) +
  facet_grid(as.factor(deli_associative$group)) + labs(y = "Colour Association Task", x = "Trial")
print(plot_deli)
```
