...
@Tabledata summarizes the estimates of learning slopes for all the treatments in each model (see Statistical analyses). On average, we found that learning slopes were lower when the 'Blue' feeders were correct compared to those assigned to the group 'Red'  for _L. delicata_ ('Blue' learning slope - 'Red'learning slope = `r format_dec(mean((dab_CORTCold + dab_ControlCold + dab_CORTHot + dab_ControlHot) - (dar_CORTCold + dar_ControlCold + dar_CORTCold + dar_ControlHot)), 3)`, p = `r pmcmc((dab_CORTCold + dab_ControlCold + dab_CORTHot + dab_ControlHot) - (dar_CORTCold + dar_ControlCold + dar_CORTCold + dar_ControlHot))`), but not for _L. guichenoti_ ('Blue' learning slope - 'Red'learning slope = `r format_dec(mean((gab_CORTCold + gab_ControlCold + gab_CORTHot + gab_ControlHot) - (gar_CORTCold + gar_ControlCold + gar_CORTCold + gar_ControlHot)), 3)`, p = `r format_dec(pmcmc((gab_CORTCold + gab_ControlCold + gab_CORTHot + gab_ControlHot) - (gar_CORTCold + gar_ControlCold + gar_CORTCold + gar_ControlHot)),3)`). However, because we suspect there may be a bias towards 'Blue' in the initial choice (see Supplementary Material and Figs. @Figdeli, @Figguich), we only used those individuals assigned to 'Red' to compare performance in the associative learning task. In this regard, we did not find any significant differences between treatments for _L. delicata_ ('Control-Cold' - 'CORT-Cold' = `r format_dec(mean(dar_ControlCold - dar_CORTCold), 3)`, p- value = `r format_dec(pmcmc(dar_ControlCold - dar_CORTCold), 3)`; 'Control-Hot' - 'CORT-Hot' = `r format_dec(mean(dar_ControlHot- dar_CORTHot), 3)`, p- value = `r format_dec(pmcmc(dar_ControlHot - dar_CORTHot), 3)`; 'Control-Hot' - 'Control-Cold' = `r format_dec(mean(dar_ControlHot - dar_ControlCold), 3)`, p-value = `r format_dec(pmcmc(dar_ControlHot - dar_ControlCold), 3)`; 'CORT-Hot' - 'CORT-Cold' = `r format_dec(mean(dar_CORTHot - dar_CORTCold), 3)`, p-value = `r format_dec(pmcmc(dar_CORTHot - dar_CORTCold), 3)`) (see @Figdeli), or _L. guichenoti_ ('Control-Cold' - 'CORT-Cold' = `r format_dec(mean(gar_ControlCold - gar_CORTCold), 3)`, p- value = `r format_dec(pmcmc(gar_ControlCold - gar_CORTCold), 3)`; 'Control-Hot' - 'CORT-Hot' = `r format_dec(mean(gar_ControlHot- gar_CORTHot), 3)`, p- value = `r format_dec(pmcmc(gar_ControlHot - gar_CORTHot), 3)`; 'Control-Hot' - 'Control-Cold' = `r format_dec(mean(gar_ControlHot - gar_ControlCold), 3)`, p-value = `r format_dec(pmcmc(gar_ControlHot - gar_ControlCold), 3)`; 'CORT-Hot' - 'CORT-Cold' = `r format_dec(mean(gar_CORTHot - gar_CORTCold), 3)`, p-value = `r format_dec(pmcmc(gar_CORTHot - gar_CORTCold), 3)`) (see @Figduich).
When groups were pooled by incubation temperature or hormonal treatment, there was not significant differences in the estimated slopes caused by temperature (_L. delicata_: 'Hot' learning slope - 'Cold' learning slope = `r format_dec(mean((dar_CORTHot + dar_ControlHot) - (dar_CORTCold + dar_ControlCold)),3)`, p-value = `r format_dec(pmcmc((dar_CORTHot + dar_ControlHot) - (dar_CORTCold + dar_ControlCold)),3)`; _L. guichenoti_: 'Hot' learning slope - 'Cold' learning slope = `r format_dec(mean((gar_CORTHot + gar_ControlHot)-(gar_CORTCold + gar_ControlCold)),3)`, p-value = `r format_dec(pmcmc((gar_CORTHot + gar_ControlHot)-(gar_CORTCold + gar_ControlCold)),3)`) or the hormone (_L. delicata_: 'Control' learning slope - 'CORT' learning slope = `r format_dec(mean((dar_ControlCold + dar_ControlHot) - (dar_CORTCold + dar_CORTHot)), 3)`, p-value = `r format_dec(pmcmc((dar_ControlCold + dar_ControlHot) - (dar_CORTCold + dar_CORTHot)), 3)` ; _L. guichenoti_: 'Control' learning slope - 'CORT' learning slope = `r format_dec(mean((gar_ControlCold + gar_ControlHot)-(gar_CORTCold + gar_CORTHot)), 3)`, p-value = `r format_dec(pmcmc((gar_ControlCold + gar_ControlHot)-(gar_CORTCold + gar_CORTHot)), 3)`). We also did not find any significant differences when we compared the estimated slopes between species for the associative task (only individuals from 'Red' group: _L. delicata_ - _L. guichenoti_ = `r format_dec(mean((dar_CORTCold + dar_ControlCold + dar_CORTHot + dar_ControlHot) - (gar_CORTCold + gar_ControlCold + gar_CORTHot + gar_ControlHot)), 3)`, p-value = `r format_dec(pmcmc((dar_CORTCold + dar_ControlCold + dar_CORTHot + dar_ControlHot) - (gar_CORTCold + gar_ControlCold + gar_CORTHot + gar_ControlHot)), 3)`).     
...
To test if lizards were biased towards blue as our preliminary analyses indicated, we employed the intercept estimates from our posterior distribution. We get this intercepts for each species, group and treatment, and we estimated the probability of choosing right using the formula: 

As we used the link=‘logit’ adjustment in our initial models.
```{r, Tablebias}
#| label: Tablebias
#| tbl-cap: "Biases"
...
prob_df <- data.frame(
  probright_drCORTCold = probright_drCORTCold,
  probright_drControlCold = probright_drControlCold,
  probright_drCORTHot = probright_drCORTHot,
  probright_drControlHot = probright_drControlHot,
  probright_dbCORTCold = probright_dbCORTCold,
  probright_dbControlCold = probright_dbControlCold,
  probright_dbCORTHot = probright_dbCORTHot,
  probright_dbControlHot = probright_dbControlHot,
  probright_grCORTCold = probright_grCORTCold,
  probright_grControlCold = probright_grControlCold,
  probright_grCORTHot = probright_grCORTHot,
  probright_grControlHot = probright_grControlHot,
  probright_gbCORTCold = probright_gbCORTCold,
  probright_gbControlCold = probright_gbControlCold,
  probright_gbCORTHot = probright_gbCORTHot,
  probright_gbControlHot = probright_gbControlHot
) %>% #This way we get a df with 16 variables
  summarise_all(.funs = mean) %>% #This way we get a df with 16 variables and 1 row
  gather(key = "column", value = "value", .) %>% #To transpose the df
  ...
  pivot_wider(names_from = Group, values_from = c(value, column)) %>% #To split the df in Red and Blue
data.frame()

write.csv(prob_df, file= "./output/Checking/prob_df.csv")

# Then we build a df with the statistical comparison between probabilities (Blue minus Red) for each treatment and species and p-values for each comparison
comp_prob <- data.frame(
  (probright_dbCORTCold - probright_drCORTCold),
  (probright_dbControlCold - probright_drControlCold),
  (probright_dbCORTHot - probright_drCORTHot),
  (probright_dbControlHot - probright_drControlHot),
  (probright_gbCORTCold - probright_grCORTCold),
  (probright_gbControlCold - probright_grControlCold),
  (probright_gbCORTHot - probright_grCORTHot),
  (probright_gbControlHot - probright_grControlHot)
) %>% 
  summarise_all(.funs = mean) %>% #This way we get a df with 16 variables and 1 row
  gather(key = "column", value = "value", .) %>% #To transpose the df
  mutate(Specie = gsub(".*_(d|g).*", "\\1",column)) %>%
  ...
  mutate(Specie = factor(Specie,
                levels = c("d", "g"),
                labels = c("d" = "L. delicata", "g" = "L. guichenoti"))) %>%
  rename("[Blue - Red]" = "value") %>%
data.frame()
#
comp_pval <- data.frame(
  pmcmc(probright_dbCORTCold - probright_drCORTCold),
  pmcmc(probright_dbControlCold - probright_drControlCold),
  pmcmc(probright_dbCORTHot - probright_drCORTHot),
  pmcmc(probright_dbControlHot - probright_drControlHot),
  pmcmc(probright_gbCORTCold - probright_grCORTCold),
  pmcmc(probright_gbControlCold - probright_grControlCold),
  pmcmc(probright_gbCORTHot - probright_grCORTHot),
  pmcmc(probright_gbControlHot - probright_grControlHot)
) %>% 
  summarise_all(.funs = mean) %>% #This way we get a df with 16 variables and 1 row
  gather(key = "column", value = "value", .) %>% #To transpose the df
  mutate(Specie = gsub(".*_(d|g).*", "\\1",column)) %>%
...
data.frame()
#Create the final database and the table
table_df <- merge(prob_df, comp_prob, by = c("Specie", "Treatment")) %>%
  merge(comp_pval, by = c("Specie", "Treatment")) %>%
  select(Specie, Treatment, value_Blue, value_Red, X.Blue...Red., p.value) %>%
  mutate(Treatment = factor(Treatment, 
                    levels = c("CORT-Cold", "Control-Cold", "CORT-Hot", "Control-Hot")))%>%
data.frame()
table_df <- table_df %>%
...
colour_table <- flextable(table_df) %>%
  bold(~ `p.value` < 0.05, ~ `p.value` + X.Blue...Red.) %>%
  set_table_properties(width = 1) %>%
  align(align="center", part="all") %>% 
  set_header_labels(value_Blue = "Blue",
                    value_Red = "Red",
                    p.value = "p-value",
                    X.Blue...Red. = "[Blue - Red]") %>%
  italic(j = 1, italic = TRUE, part = "body") %>% # To have names od species in italics
  colformat_num(columns = c("Blue", "Red", "[Blue-Red]", "p-value"), digits = 3) %>% # To set the number of decimals
...
For _L. delicata_, there was no differences between treatments ('Red' group: 'Control-Cold' - 'CORTCold')
