...
data_associative <- data %>%
  group_by(lizard_id) %>%
  mutate(
    trial_associative = ifelse(!is.na(trial_associative), min(trial_associative, na.rm = TRUE), NA),
    trial_associative = trial_associative - min(trial_associative, na.rm = TRUE) + 1
  ) %>%
  ungroup()

# Split data
deli <- data_associative %>% filter (species == "delicata")
guich <- data_associative %>% filter (species == "guichenoti")
