...
# Remove individuals who did not participate (more than 15 NAs), remove trials [36-40] and split treatment into Temp and Cort
data_clean <- data %>%
  group_by(lizard_id) %>%
  ...
  ungroup()  %>%
    ...
    mutate( # Standarize data by trial (i.e. make the first trial where each individual participated their trial 1)
      ...
      Trial = ifelse(!is.na(first_non_na),trial_associative - first_non_na + 1, trial_associative))%>% 
      filter(Trial >= 1) %>%
  ungroup() %>% 
data.frame()