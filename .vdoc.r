...
specs <- getspec(here("/Others/Feeders"), lim = c(300,700), ext = 'txt')
specs <- procspec(pablospecs, fixneg = "addmin")
#
#First rename the measurements for ease of combination
colnames(pablospecs) <- c("wl", "blue_1", "blue_2", "blue_3", "blue_4", "blue_5", "green_1", "green_2", "white_1", "white_2", "white_3", "green_3", "green_4","green_5", "white_4", "white_5")
#
# Figure out best smoothing option
## 0.2 smoothing is best (seems like this is usually the case)
plotsmooth(pablospecs,
  minsmooth = 0.05,
  maxsmooth = 0.5,
  curves = 4,
  ask = FALSE
...
#
# Process data for smooth spectral curves, fix negative reflectance and prepare for plotting and visual modeling. 
pablospecs_smoothed<-procspec(pablospecs, opt = "smooth" , span = 0.2, fixneg = 'addmin')
#
# Plot smoothed raw data as reflectance curve with approximate rgb colours
plot(pablospecs_smoothed, type = "o", col = spec2rgb(pablospecs_smoothed))
#
## Alternative approach - plotting the repeated measurements of each colour together:
## This code creates a vector of colour identities
## As the format of the data prevents having identifying tags in the dataset, the names of the files need to have identifying strings that we can use to group
## This code starts by looking at the names, removing the ID number (e.g. _3) and then grouping by the first part of the name which is the feeder colour
colourgroups <- gsub("_[0-9].*$", "", names(pablospecs_smoothed)) [-1]
## Check this worked 
table(colourgroups)
#
## Plot using median and standard deviation, edited colours and thicker lines (part 2.3.2 of the pavo handbook)
aggplot(pablospecs_smoothed, colourgroups, FUN.center = median, alpha = 0.3, legend = TRUE, lcol = c("royalblue", "seagreen", "lightyellow1"), shadecol = "grey", lwd = 3)
#
## Processing the data with the lizard visual model 
# This is the complicated bit!
# very basic processing using lizard vision model provided for Dan and Martin's Phyrnocephalus mystaceus paper
liz_vis <- sensmodel(c(360, 440, 493, 571)) 
#
# Apply this over the feeder data visual is the visual system, achromatic is the receptor sensitivity, illum is the illuminate (here using an ideal white illuminate preset), bkg is the background against which we are measuring (here using an ideal background with no influence). Other conditions are applied as used in the P mystaceus paper and I don't understand them yet.
...
feeders_lizardview <- vismodel(pablospecs_smoothed, visual = liz_vis, achromatic = "l", illum = "ideal", bkg = "ideal", vonkries = TRUE,   relative = FALSE, qcatch = "fi")
#                             
...
#
# Remove duplicate categories(e.g white/green is the same as green/white and all are called green/white)
annotated_coldistance_feeders <- annotated_coldistance_feeders %>%
  mutate(color_comparison = paste0(pmin(color1, color2), "/", pmax(color1, color2)))
#
...
dScomparisons <- ggplot(annotated_coldistance_feeders, aes(x = color_comparison, y = dS, fill = color_comparison)) + 
 ...
 scale_fill_manual(values =c("royalblue1", "cyan", "steelblue1", "seagreen", "darkseagreen3", "white"))+ theme(legend.position = "none")
dScomparisons
